...
class User_info_model extends CI_Model {
	...
	function check_my($uid){
		$data=$this->db->select('pro_title','pro_goal','pro_status','pro_dur','pro_class','pro_start')->from('pro_info')->where(array('user_id'=>$uid))->order_by('pro_id','asc')->get()->result_array();
	}
	/**
	 * 所发起项目个数
	 */
	function  my_pro($uid){
		$data=$this->db->select_sum('pro_id')->from('pro_info')->where(array('user_id'=>$uid))->get()->result_array();
		if($data){
	...
	function check_don_all($uid){
		$data=$this->db->select_sum('don_money')->from('donate')->where(array('user_id'=>$uid))->get()->result_array();
		if($data){
			return $data;
		}else{
			return 0;
